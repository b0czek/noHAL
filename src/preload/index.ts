...
import type { ComponentStore, ComponentStoreEntry, ImportedComponentDefinition, NoHALProject } from "../shared/types";
...
const api: NoHALApi = {
  ...
  loadComponentStore: () => ipcRenderer.invoke("nohal:load-component-store") as Promise<ComponentStore>,
  importCompFileToStore: () =>
    ipcRenderer.invoke("nohal:import-comp-file-to-store") as Promise<ComponentStoreEntry | null>,
  ...
  refreshComponentSourceInStore: (sourceId) =>
    ipcRenderer.invoke("nohal:refresh-component-source-in-store", sourceId) as Promise<{
      sourceId: string;
      entries: ComponentStoreEntry[];
      removedComponentIds: string[];
      errors: Array<{ filePath: string; error: string }>;
  ...
  deleteComponentSourceFromStore: (sourceId) =>
    ipcRenderer.invoke("nohal:delete-component-source-from-store", sourceId) as Promise<{
      sourceId: string;
      removedComponentIds: string[];
    }>,
  refreshComponentInStore: (componentId) =>
    ipcRenderer.invoke("nohal:refresh-component-in-store", componentId) as Promise<ComponentStoreEntry>,
