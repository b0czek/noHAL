...
- when using `neverthrow` for new result-style interfaces, prefer `Result<Change<T>, Failure<...>>` with object errors using `code`; see `packages/core/src/result.ts` for the shared `Change`, `Failure`, and `matchFailure` helpers, and avoid string errors and ad-hoc wrapper abstractions around `neverthrow`
